...
var GameType = -1,
  ...
  GameTime = 180000 /*3 Minutes in Milliseconds*/,
  GameStart = -1;
var GameTypes = [
  ["普通", "玩一場常規遊戲"],
  ["時間攻擊", "3分鐘盡可能獲取多的積分"],
...
function SendGameTypeSelectionMenu() {
  ...
  for (var i = 0; i < GameTypes.length; i++) {
    SendStr +=
      "#L" + i + "#" + GameTypes[i][0] + " - " + GameTypes[i][1] + "#l\r\n";
  }
...
function GetGameSettingsDescriptionString() {
  return (
    ...
    " - 大小: " +
    BoardSizes[BoardSize][0] +
    "(" +
    BoardSizes[BoardSize][1] +
    "x" +
    BoardSizes[BoardSize][2] +
    ") - 難易度: " +
...
function RemoveAndGenerateNewGems(Combo, r, c, GemGroup, IsSpecialItem) {
  var DropItemProb = IsSpecialItem
    ? 0
    : GameDropProbability +
      (Combo.length > 5 ? (GameDropProbability / 10) * (Combo.length - 5) : 0);
  var SpecialItemProb = IsSpecialItem
    ? 0
    : 0.005 +
      (Combo.length > 3 ? 0.01 * (Combo.length - 3) : 0.0) +
      DropItemProb;
  for (var i = 0; i < Combo.length; i++) {
    var TurnToGemProb =
      0.2 + (Combo.length > 3 ? 0.2 * (Combo.length - 3) : 0.0);
    if (
      Combo[i][0] == r &&
      Combo[i][1] == c &&
      GemGroup != null &&
      GemGroup[0] == Board[r][c] &&
      Math.random() < TurnToGemProb
    ) {
...
function IsOreOrGem(ItemID) {
  for (var i = 0; i < Gems.length; i++) {
    for (var j = 0; j < Gems[i].length; i++) {
      if (Gems[i][j] == ItemID) return true;
    }
  }
  return false;
}
...
function AddScoreForCombination(Combo, GemGroup) {
  var Multiplier =
    1.0 +
    (Combo.length > 3 ? 0 : 0.05 * (Combo.length - 3)) +
    NextComboMultiplier;
  if (Combo.length == 3) {
    NextComboMultiplier = 0;
...
function GetTimeLeftString() {
  ...
  Ret +=
    ...
    "." +
    (MilliSec < 100
      ? "0" + (MilliSec < 10 ? "0" + MilliSec : MilliSec)
      : MilliSec);
  Ret += " seconds\r\n";
  return Ret;
}
...
function action(mode, type, selection) {
  if (mode == -1 || mode == 0) cm.dispose();
    ...
    if (status == 0) {
      ...
      GameType = selection; //Choose Board Size
      ...
      BoardSize = selection; //Choose Difficulty
      ...
      InitializeBoard();
      InitializeGems(); //No Header while in game, it's just annoying
      cm.sendSimple(
        GetScoreString() +
      ...
      if (NoMoreMoves()) {
        ...
        cm.sendSimple(
          (ValidMove
            ? ""
            : "無效的移動，您必須單擊一組3個或更多的礦石/寶石。\r\n") +
            GetScoreString() +
            (GameType == 1 ? GetTimeLeftString() : "") +
            GetBoardSelectionString() +
            GetGameSettingsDescriptionString()
        );
      }
